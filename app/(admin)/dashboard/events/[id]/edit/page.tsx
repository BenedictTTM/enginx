...
export default function EditEventPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    const fetchEvent = async () => {
      try {
        const res = await apiClient.getEventBySlug(id);
        if (res.success && res.data) {
          ...
          setImagePreview(event.featuredImage || null);
        } else {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6 pb-12 bg-[#071a35] text-white">
      <div className="flex items-center space-x-4">
        ...
        <h1 className="text-2xl font-bold tracking-wide text-white">Edit Event</h1>
      </div>

      <form onSubmit={handleSubmit} className="bg-[#0c2546]/80 border border-[#163b6b]/60 rounded-2xl shadow-lg p-6 sm:p-8 space-y-6">
        {error && (
          <div className="p-4 bg-red-500/10 text-red-400 rounded-xl border border-red-500/20 text-sm whitespace-pre-line">
            {error}
          </div>
        )}
        ...
        <div className="space-y-4">
          ...
          <div className="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-[#163b6b] border-dashed rounded-2xl relative overflow-hidden group hover:border-[#8dbef8] transition-all duration-200">
            ...
            <input id="file-upload" name="file-upload" type="file" accept="image/*" className="absolute inset-0 w-full h-full opacity-0 cursor-pointer" onChange={handleImageChange} />
          </div>
        </div>
        ...
        <div className="pt-6 flex justify-end">
          <button type="submit" disabled={saving} className="bg-white hover:bg-gray-100 text-[#021128] font-bold px-6 py-3 rounded-xl transition-all duration-200 flex items-center disabled:opacity-50 cursor-pointer shadow-md">
            {saving ? <Loader2 className="w-5 h-5 animate-spin mr-2 text-[#021128]" /> : null}
            Save Changes
          </button>
        </div>
      </form>
